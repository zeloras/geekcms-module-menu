<form method="POST" action="{{ route('admin.menu.item.save', ['item' => $item->id ?? null]) }}">
    ...
    <div class="form-group">
        ...
        <input class="form-control" type="url" id="iActionLink" value="{{ array_get($item->action ?? '', 'link') }}"
               placeholder="https://google.com">
    </div>

    <div class="form-group">
        <label for="iActionAnhor">{{ Translate::get('module_menu::admin/main.anchor') }}:</label>
        <input class="form-control" id="iActionAnhor" type="text" value="{{ array_get($item->action ?? '', 'anhor') }}"
               placeholder="#title">
    </div>
            ...
            @foreach($pages as $page)
                @php
                    $selected = ($item && 'page.open' === array_get($item->action, 'route')
                        ...
                        : '';
                @endphp
                <option value="route::page.open|page={{ $page->slug }}" {{$selected}}>{{ $page->name }}
                    ({{ $page->lang }})
                </option>
            @endforeach
        </select>
    ...
    @if (isset($item) && !empty($item))
        <button id="itemCreate"
                class="btn btn-primary">{{ Translate::get('module_menu::admin/main.change_button') }}</button>
    @else
        <button id="itemCreate"
